...
function verifyStripeSignature(raw: string, header: string, secret: string, toleranceSec = 300) {
...
export async function POST({ request }: { request: Request }) {
  ...
  if (!sigHeader || !verifyStripeSignature(rawBody, sigHeader, secret, 300)) {
    try { console.warn(`[${new Date().toISOString()}] stripe_signature_invalid`); } catch {}
    const h = secHeaders({ 'content-type': 'application/json' });
    return new Response(JSON.stringify({ ok: false, error: 'signature_invalid' }), { status: 400, headers: h });
  }

  // Signature verified → parse event safely
  try {
    const event = JSON.parse(rawBody);
    switch (event.type) {
      case 'checkout.session.completed': {
        // TODO: persist/notify as needed (Slack/DB). Success page is handled by Checkout success_url.
        break;
      }
      default:
        break;
    ...
    try { console.error(`[${new Date().toISOString()}] stripe_event_invalid_json`); } catch {}
    const h = secHeaders({ 'content-type': 'application/json' });
